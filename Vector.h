...
//////////////////////METHODS//////////////////////
	...
	void assign(size_type size, const_reference value) {
		if (size < 0) throw std::out_of_range {"Vector::assign"};
		delete[] elem_;
		size_ = size;
		capacity_ = size_;
	...
	reference at(size_type pos) {
		if (pos < 0 || size() < pos) throw std::out_of_range {"Vector::at_ref"};
		return elem_[pos];
	}
	const_reference at(size_type pos) const {
		if (pos < 0 || size() < pos) throw std::out_of_range {"Vector::at_c_ref"};
		return elem_[pos];
	}
...
//////////////////////OPERATORS//////////////////////
	...
	const_reference operator[](size_type i) const {
		if (i < 0 || capacity() <= i) throw std::out_of_range {"Vector::operator[]_c"};
		return elem_[i];
	}
	//coppy assignmment
